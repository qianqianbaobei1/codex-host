...
describe("Cursor native configuration", () => {
  it("starts inspection cache expiry at completion, including slow native startup", async () => {
    ...
    // This suite pins the ACP cache contract; the lightweight catalog path is
    // covered separately. Force the fallback so the transport spy drives it.
    vi.spyOn(adapter, "discoverCatalogLightweight").mockRejectedValue(
      new Error("lightweight discovery is not exercised here"),
    );
    try {
