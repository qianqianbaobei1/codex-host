...
export class CursorAdapter implements HarnessAdapter {
  ...
  /**
   * Discover the account Model catalog through `cursor-agent models`.
   *
   * Measured on a 5 GB / 85k-file workspace: the subcommand answers in ~2.4s,
   * while opening a full ACP session just to read the catalog costs ~14s
   * (initialize + authenticate + newSession against the workspace). Only the
   * catalog fields cursorCatalog needs are derived here; full sessions still
   * use ACP.
   */
  async discoverCatalogLightweight(cwd: string): Promise<HarnessModelCatalog> {
    const environment = this.options.environment ?? process.env;
    const invocation = cursorInvocation(environment, this.options.command);
    const { stdout } = await promisify(execFile)(invocation.command, ["models"], {
  ...
  async inspect(input: InspectHarnessInput = {}): Promise<HarnessInspection> {
    ...
    const result = (async (): Promise<HarnessInspection> => {
      try {
        const catalog = await this.discoverCatalogLightweight(cwd);
        return {
          status: "ready",
          catalog,
        ...
        const lightweightFailure = cursorError(lightweightError);
        // A missing/unauthorised CLI cannot be rescued by opening a session.
        if (
          lightweightFailure.code === "notInstalled" ||
          lightweightFailure.code === "authenticationRequired"
        ) {
          return { status: "notInstalled", error: lightweightFailure };
        }
      }
      // Fallback: derive the catalog from a full ACP session. Slower (~14s on
      // large workspaces) but works for CLI builds without the subcommand.
