...
describe("current Codex Renderer Agent adapter", () => {
  ...
  it("isolates thread-scoped applyAgent from draft desiredCarrier", () => {
    const priorWindow = Object.getOwnPropertyDescriptor(globalThis, "window");
    const priorDocument = Object.getOwnPropertyDescriptor(globalThis, "document");
    const priorCustomEvent = Object.getOwnPropertyDescriptor(globalThis, "CustomEvent");

    const policySelect = vi.fn(() => true);
    const mockBridge = {
      hostId: "local",
      sendRequest: vi.fn(),
      prewarmThreadStart: vi.fn(),
    ...
    const fakeWindow = {
      __codexhostDraftPrewarmPolicyV1: mockPolicy,
    ...
    try {
      const adapter = installCurrentRendererAdapter();
      adapter.applyAgent("codex");
      expect(policySelect).toHaveBeenLastCalledWith(null, undefined);

      const composerElement = composerWithFiber({});
      addComposerPortal(composerElement, "thread-1");
      const model = harnessModelRefSchema.parse({ id: "gemini-3.8-flash" });
      adapter.applyAgent("antigravity", model, undefined, undefined, composerElement);
      expect(policySelect).toHaveBeenLastCalledWith(expect.stringContaining("gemini-3.8-flash"), "thread-1");

      const composer2 = composerWithFiber({});
      addComposerPortal(composer2, "thread-2");
      adapter.applyAgent("codex", undefined, undefined, undefined, composer2);
      expect(policySelect).toHaveBeenLastCalledWith(null, "thread-2");
